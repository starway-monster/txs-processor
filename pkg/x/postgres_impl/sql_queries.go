...
func addTxStats(stats types.TxStats) string {
	return fmt.Sprintf(addTxStatsQuery,
		fmt.Sprintf("('%s', '%s', %d)", stats.ChainID, stats.Hour.Format(types.Format), stats.Count),
		stats.Count,
	)
}
			...
			for dest, hourMap := range destMap {
				for hour, count := range hourMap {
					queries = append(queries, fmt.Sprintf(addIbcStatsQuery,
						fmt.Sprintf("('%s', '%s', '%s', '%s', %d)", origin, source, dest, hour.Format(types.Format), count),
						count))
				}
			}
		}
