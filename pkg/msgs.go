...
func processMsgs(ctx context.Context, block types.Block, builder *builder.MutationBuilder, msgs []sdk.Msg) error {
	...
	for _, msg := range msgs {
		...
		case transfer.MsgTransfer:
			chainID, err := chainID(ctx, chainIDs, clientIDs, connectionIDs, block.ChainID, msg.SourceChannel)
			if err != nil {
				return err
			}
		...
		case channel.MsgPacket:
			chainID, err := chainID(ctx, chainIDs, clientIDs, connectionIDs, block.ChainID, msg.DestinationChannel)
			if err != nil {
				return err
			}
...
func chainID(ctx context.Context, chainIds, clientIDs, connectionIDs map[string]string, source, channel string) (string, error) {
...
func upsertIbcStats(ctx context.Context, b *builder.MutationBuilder, transfers map[string]bool, source, chainID string, t time.Time) error {
	...
	localExists := transfers[source+chainID]
